...
(() => {
  ...
  function draw(){
    ...
    const startX = isMobileView ? W * .095 : W * .13;
    const founderX = isMobileView ? W * .915 : W * .89;
    // Threadguy can never visually touch Founder.
    const unreachableX = founderX - (isMobileView ? W * .11 : W * .086);
    ...
    const founderH = isMobileView ? Math.min(H * .64, 210) : Math.min(H * .73, 286);
    const threadH = isMobileView ? Math.min(H * .56, 182) : Math.min(H * .64, 245);
