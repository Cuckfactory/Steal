...
(() => {
  ...
  if(music){music.volume=.28;music.loop=true;}
  function startMusic(){
    if(!music||document.hidden)return;
    musicStarted=true;
    try{const attempt=music.play();attempt?.catch(()=>{});}catch{}
  }
  function pauseMusic(){if(!music)return;try{music.pause();}catch{}}
  ...
  // Canonical rule: Threadguy can NEVER reach Founder.
  // Even the theoretical best run leaves at least 0.75% distance.
  const HARD_MAX_PROGRESS = 0.9925;
  ...
  function remainingPct(){
    return Math.max(100 * (1 - HARD_MAX_PROGRESS), 100 * (1 - state.progress));
  }

  function calcPower(){
    // Distance remains dominant. Pace separates otherwise similar runs.
    const p = Math.max(0, Math.min(HARD_MAX_PROGRESS, state.progress));
  ...
  function lateDifficulty(){
    ...
    return Math.pow(Math.max(0, Math.min(1, state.progress)), 1.95);
  }
  ...
  function progressSpeed(){
    ...
    const remainingToCeiling = Math.max(0, HARD_MAX_PROGRESS - state.progress);
    return 0.215 * remainingToCeiling;
  }
  ...
  function founderTurnSpeed(){
    const d = lateDifficulty();
    return (2.35 + 3.95*d) * state.turnSpeedFactor;
  }
  ...
  function founderTurnToWatch(){
    ...
    // Most inspections vary moderately; about one in six is a much faster snap-check.
    const snapCheck = rnd(0,1) < .17;
    state.turnSpeedFactor = snapCheck ? rnd(1.42,1.78) : rnd(.78,1.28);
  ...
  addEventListener('pagehide',pauseMusic);
  ...
  function update(dt){
    ...
    if(state.moving){
      ...
      state.progress = Math.min(HARD_MAX_PROGRESS - 0.000001, state.progress + progressSpeed()*dt);
      state.power = calcPower();
    }
    updateHUD();
  }
  ...
  function draw(){
    ...
    const isMobileView = W < 700;
    const floorY = H * .93;
    // Same underlying score/progress on every device, but mobile gets a slightly
    // wider-feeling runway and smaller sprites so it LOOKS as fair as desktop.
    const startX = isMobileView ? W * .07 : W * .13;
    const founderX = isMobileView ? W * .945 : W * .89;
    // Threadguy can never visually touch Founder.
    const unreachableX = founderX - (isMobileView ? W * .145 : W * .086);
    ...
    // Mobile gets slightly smaller characters so the stage does not feel zoomed in.
    const founderH = isMobileView ? Math.min(H * .58, 192) : Math.min(H * .73, 286);
    const threadH = isMobileView ? Math.min(H * .50, 164) : Math.min(H * .64, 245);
  ...
  function drawScorecard(r){
    ...
    c.textAlign='center';c.shadowColor='#100805';c.shadowBlur=15;c.shadowOffsetY=3;c.fillStyle='#fff1ce';c.font='700 84px Oswald';c.fillText('THREADGUY SNIFF CHASE',600,118);
  ...
  function downloadCard(){
    try{
      ...
      a.download=`threadguy-sniff-chase-${state.power}-cuck-power.png`;
  ...
  function shareX(){
    const best=JSON.parse(localStorage.getItem('sniff-best')||'null');
    if(!best) return;
    const text=`I got within ${best.nearest.toFixed(2)}% of the Founder and scored ${best.power.toLocaleString('en-US')} CUCK POWER in THREADGUY SNIFF CHASE.\n\nGet to work: cucks.money`;
    window.open(`https://x.com/intent/post?text=${encodeURIComponent(text)}`,'_blank','noopener,noreferrer');
  }

  // Prevent iPhone long-press selection / copy UI inside the game shell.
  ['contextmenu','selectstart'].forEach(type=>{
    gameShell?.addEventListener(type,e=>e.preventDefault());
  });
  ...
  addEventListener('keydown',e=>{
    if(e.code==='Space'){
      e.preventDefault();
      if(!e.repeat){startMusic();setMove(true);}
    }
  });
  ...
  $('startBtn').addEventListener('click',()=>{startMusic();start();});
  ...
  addEventListener('pointerup',()=>setMove(false));
  addEventListener('touchend',()=>setMove(false), {passive:true});
